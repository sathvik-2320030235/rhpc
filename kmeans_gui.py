...
class KMeansGUI:
    ...
    def compile_code(self):
        if os.path.exists("./build.sh"):
            self.run_process_async(["bash", "./build.sh"])
            return

        # Fallback: try to build what we can (Linux/macOS vs Windows)
        if sys.platform.startswith("win"):
            cmd = ["powershell.exe", "-ExecutionPolicy", "Bypass", "-File", ".\\build.ps1"]
            self.run_process_async(cmd)
        else:
            cmd = ["bash", "-lc", "g++ -O3 -std=c++11 kmeans_serial.cpp -o kmeans_serial && g++ -O3 -std=c++11 -fopenmp kmeans_omp.cpp -o kmeans_omp"]
            self.run_process_async(cmd)
    ...
    def run_code(self):
        ...
        if self.mode_var.get() == "Serial":
            exe = "kmeans_serial.exe" if sys.platform.startswith("win") else "./kmeans_serial"
            if not os.path.exists(exe):
                messagebox.showerror("Error", f"{exe} not found. Compile first!")
                return
            self.run_process_async([exe, n, d, k, iters])
        else:
            mpi_ranks = self.entry_mpi.get()
            omp_threads = self.entry_omp.get()

            exe = "kmeans_parallel.exe" if sys.platform.startswith("win") else "./kmeans_parallel"
            ...
            self.run_process_async(["mpiexec", "-n", str(mpi_ranks), exe, n, d, k, iters], env=env)
